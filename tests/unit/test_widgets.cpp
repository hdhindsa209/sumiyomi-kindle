...
void test_golden_cover_grid()
{
    ...
    int32_t cw = 0, ch = 0;
    cover_size(3, kW, cw, ch);
    auto px = std::make_shared<std::vector<uint8_t>>(static_cast<size_t>(cw) * static_cast<size_t>(ch));
    for (int32_t y = 0; y < ch; ++y)
        for (int32_t x = 0; x < cw; ++x) (*px)[static_cast<size_t>(y * cw + x)] = static_cast<uint8_t>((y * 16 / ch) * 17);
    covers[0].image = px;
