...
cp "$ROOT/tools/kual/config.xml" "$ROOT/tools/kual/menu.json" "$ROOT/tools/kual/run.sh" \
   "$ROOT/tools/kual/uninstall-app.sh" "$ROOT/tools/kual/uninstall-all.sh" \
   "$ROOT/tools/kual/uninstall-run.sh" "$PKG/"
...
"$ROOT/tools/kbuild.sh" sh -c 'for b in sumiyomi aix_runner; do arm-kindlehf-linux-gnueabihf-strip -o build/package/sumiyomi/bin/$b build/kindle/bin/$b; done'
chmod +x "$PKG/bin/sumiyomi" "$PKG/bin/aix_runner"
